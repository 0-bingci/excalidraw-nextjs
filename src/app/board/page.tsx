...
export default function ExcalidrawClone() {
  ...
  // 3. 发送消息（更新消息列表，清空输入框）
  // 1. 修改 AiDialog 状态结构，增加 metadata 字段
  const [aiDialog, setAiDialog] = useState<{
    ...
    input: "",
    messages: [],
    metadata: null, // 初始化为 null
  });

  // 2. 右键菜单触发 AI 对话框（保存当前画布状态）
  const handleOpenAiDialog = () => {
    ...
    setAiDialog((prev) => ({
      ...
      x: contextMenu.x,
      y: contextMenu.y,
      metadata: currentCanvasJson, // 👈 保存当前画布数据
      messages: [], // 可选：清空历史消息
    }));
  };
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.type === "application/json" || file.name.endsWith(".json")) {
      reader.onload = (event) => {
        ...
        canvas.current?.loadFromJSON(jsonContent, () => {
          canvas.current?.renderAll();
        });
      };
    ...
    switch (property) {
      ...
      case "copy":
        try {
          ...
          selectedObject
            .clone()
            .then((clonedObj: fabric.Object) => {
              // 调整克隆对象位置（向右下方偏移20px，避免重叠）
              if (
                typeof clonedObj.left === "number" &&
                typeof clonedObj.top === "number"
              ) {
                clonedObj.set({
            ...
            .catch((error) => {
              ...
              if (canvas.current) {
                ...
                fabric.util.enlivenObjects([json], (objects) => {
                  ...
                  if (
                    typeof clonedObject.left === "number" &&
                    typeof clonedObject.top === "number"
                  ) {
        ...
        if (selectedObject.group) {
          ...
          if (index !== -1 && index < group._objects.length - 1) {
            ...
            [group._objects[index], group._objects[index + 1]] = [
              group._objects[index + 1],
              group._objects[index],
            ];
            group.renderAll();
          }
        } else {
  ...
  useEffect(() => {
    ...
    canvasToolsRef.current = initializeCanvasTools(
      canvas.current,
      (tool: Tool) => {
